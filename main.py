...
def enqueue_video(video_id: str = "AAA"):
    ...
    return {
        "task_id": task.id,
        "message": f"Video {video_id} has been queued for processing.",
    }
...
def create_user(email: str, password: str, name: str):
    ...
    if result:
        ...
        return JSONResponse(
            status_code=422,
            content=(Error(error="true", message="此信箱已被使用").dict()),
        )
    else:
...
def authenticate_user(email: str, password: str):
    ...
    if not result:
        ...
        return JSONResponse(
            status_code=401,
            content=(
                Error(error="true", message="使用者帳號/密碼不匹配，請再試一次").dict()
            ),
        )
...
async def signup(
    sign_up_credentials: SignupFormData, request: Request, response: Response
):
    return create_user(
        email=sign_up_credentials.email,
        password=sign_up_credentials.password,
        name=sign_up_credentials.name,
    )


@app.put("/api/user/auth", response_model=TokenOut, summary="登入會員帳戶")
async def signin(
    sign_in_credentials: SigninFormData, request: Request, response: Response
):
    return authenticate_user(
        email=sign_in_credentials.email, password=sign_in_credentials.password
    )
...
@app.get(
    "/api/user/auth",
    response_model=UserSigninDataOut,
    summary="取得當前登入的會員資訊",
    tags=["User"],
)
async def check_signin_status(
    request: Request,
    response: Response,
    token_data: TokenOut = Depends(get_token_header),
):
...
def get_new_api_key(token_data):
    ...
    if not check_user_signin_status_return_bool(token_data):
        return JSONResponse(
            status_code=403,
            content={"error": True, "message": "使用者驗證失敗，請先登入"},
        )
    else:
        try:
            website_db = mysql.connector.connect(
...
async def process_fake_video(
    request: Request, response: Response, video_info: VideoParseInfo
):
    # class VideoParseInfo(BaseModel):
    # youtube_id: str
    # api_key: str
    ...
    if api_key_result:
        ...
        task = celery_config.process_video.delay(
            video_info.youtube_id, video_info.api_key
        )
        return {
            "ok": True,
            "task_id": task.id,
            "message": f"Video {video_info.youtube_id} has been queued for processing.",
        }
        ...
        return JSONResponse(
            status_code=401,
            content=(
                Error(error="true", message="API key不正確，請確認輸入資訊").dict()
            ),
        )
    ...
    try:
        ...
        if not signin_status:
            ...
            if file_size > MAX_FILE_SIZE:
                return JSONResponse(
                    status_code=413,
                    content=(Error(error="true", message="檔案大小超出上限").dict()),
                )
            ...
            website_db_cursor.execute(
                cmd, (user_id, upload_video_id, file_url, "s3", gameType)
            )
            website_db.commit()
            ...
            if processing_items_result >= 3:
                return {
                    ...
                    "message": "目前請求數量超出伺服器上限，已上傳此影片，但未開始分析，請稍待再送出分析需求。",
                }
            # If not too many items, start the process
                ...
                if db_fetch_result:
                    ...
                    task = celery_config.process_uploaded_video.delay(
                        upload_video_id, user_id, gameType
                    )
                    return {
                        "ok": True,
                        "filename": upload_video_id,
                        "message": f"已經排入處理佇列，請至會員中心確認結果",
                    }
...
async def get_uploaded_videos(token_data: TokenOut = Depends(get_token_header)):
    ...
    if not signin_status:
        return JSONResponse(
            status_code=401,
            content=(Error(error="true", message="登入資訊異常，請重新登入").dict()),
        )
    else:
        user_id = signin_status["id"]
        website_db = mysql.connector.connect(
...
async def process_video_by_id(
    process_info: VideoParseInfoUploaded,
    token_data: TokenOut = Depends(get_token_header),
):
    print(process_info)
    return {"ok": True}
    ...
    # 檢查video_id & API key狀態
        ...
        if processing_items_result >= 3:
            return JSONResponse(
                status_code=500,
                content=(
                    Error(error="true", message="目前有太多請求，請稍後再試").dict()
                ),
            )
        ...
        if not db_fetch_result:
            return JSONResponse(
                status_code=401,
                content=(
                    Error(error="true", message="影片資訊不正確，請重新確認").dict()
                ),
            )
            ...
            return JSONResponse(
                ...
                content=(
                    Error(
                        error="true", message="影片正在處理中，請至會員中心確認結果"
                    ).dict()
                ),
            )
            ...
            return JSONResponse(
                ...
                content=(
                    Error(
                        error="true", message="影片已處理完畢，請至會員中心確認結果"
                    ).dict()
                ),
            )
        # status = "UPLOADED", 可以處理影片的狀態
            ...
            task = celery_config.process_uploaded_video.delay(
                video_unique_id, user_id, game_type
            )
            return {
                "ok": True,
                "filename": video_unique_id,
                "message": f"已經排入處理佇列！`",
            }


@app.post("/api/process_uploaded_video_dummy", summary="解析從S3上傳的影片")
async def process_video_dummy(
    process_info: VideoParseInfoUploaded,
    token_data: TokenOut = Depends(get_token_header),
):
    # 檢查登入狀態
    ...
    # 檢查video_id & API key狀態
        ...
        website_db_cursor.execute(
            cmd, (user_id, upload_video_id, file_url, "NOT PROCESSED")
        )
        website_db.commit()
        ...
        if not db_fetch_result:
            return JSONResponse(
                ...
                content=(
                    Error(
                        error="true", message="影片資訊或API key不正確，請重新確認"
                    ).dict()
                ),
            )
            ...
            task = celery_config.process_uploaded_video_dummy.delay(
                uploaded_video_id, api_key
            )
            return {
                "ok": True,
                "task_id": task.id,
                "video_id": uploaded_video_id,
                "message": f"已經排入處理隊列，請至會員中心確認結果",
            }
...
async def patch_user_preferences(
    user_preferences: UserPreferences, token_data: TokenOut = Depends(get_token_header)
):
    # 檢查登入狀態
    signin_status = check_user_signin_status_return_bool(token_data)
    if not signin_status:
...
async def test_cicd():
    return {
        "ok": True,
        "message": "If you see this message, CI/CD pipeline is working!",
    }
...
async def validation_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=400,
        content=(Error(error="true", message="格式不正確，請確認輸入資訊").dict()),
    )
...
async def internal_server_error_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content=(
            Error(error="true", message="伺服器內部異常，請聯繫管理員確認").dict()
        ),
    )